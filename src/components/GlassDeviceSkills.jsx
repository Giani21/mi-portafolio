    ...
    <section className="py-16 sm:py-24 md:py-32 px-4 sm:px-6 min-h-screen bg-[#030712] relative overflow-hidden flex flex-col items-center">
      ...
      <div className="max-w-7xl mx-auto relative z-10 w-full">
        ...
        <div className="sm:hidden relative">
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            ...
            >
              <FaChevronLeft className="text-green-400 text-sm" />
            </button>
            ...
            <button
              ...
              className="w-10 h-10 flex items-center justify-center border border-white/10 bg-zinc-900/50 rounded hover:bg-white/5 transition-colors"
            >
              <FaChevronRight className="text-green-400 text-sm" />
            </button>
          </div>
        </div>
        ...
        {/* Desktop with reflections (> 1024px) */}
      ...
      {selectedId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-5xl bg-[#0a0c14]/90 backdrop-blur-2xl rounded-lg sm:rounded-xl border border-white/10 shadow-2xl flex flex-col md:flex-row overflow-hidden animate-modalScale max-h-[90vh] md:max-h-[80vh]">
            
            {/* Left side - Icon & Category */}
            <div className={`w-full md:w-1/2 flex flex-col items-center justify-center p-8 sm:p-12 border-b md:border-b-0 md:border-r border-white/5 relative overflow-hidden group/hud`}>
              ...
              <div className="hidden md:block absolute left-4 top-1/2 -translate-y-1/2 font-mono text-[7px] text-green-500/40 uppercase [writing-mode:vertical-lr] tracking-[1em]">
                {hudText.protocol}
              </div>

              <div className="relative animate-glitchEntry">
                <div className={`absolute inset-0 blur-2xl sm:blur-3xl opacity-30 bg-${currentCategoryColor}-500 animate-pulse`} />
                {iconMap[selectedSkill.iconName] &&
                  React.createElement(iconMap[selectedSkill.iconName], {
                    className: `w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 text-${currentCategoryColor}-400 mb-4 sm:mb-6 relative z-10 drop-shadow-[0_0_15px_rgba(0,0,0,0.5)]`,
                  })}
              </div>
              
              <h3 className="text-3xl sm:text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter relative z-10 animate-slideUpFade text-center">
                {selectedSkill.category}
              </h3>
            </div>

            {/* Right side - Skills list */}
            <div className="w-full md:w-1/2 p-6 sm:p-8 md:p-12 flex flex-col justify-center gap-3 sm:gap-4 bg-[#0a0c14]/50 overflow-y-auto">
              <span className="font-mono text-[8px] sm:text-[9px] text-green-500/60 tracking-[0.3em] sm:tracking-[0.4em] mb-2 sm:mb-4 uppercase">
              ...
              {selectedSkill.items.map((item, i) => (
                ...
                >
                  <span className="text-sm sm:text-base text-zinc-200 group-hover/item:text-white transition-colors">{item}</span>
                  <span className={`text-green-500/50 font-mono text-[9px] sm:text-[10px] self-end italic`}>{hudText.ready}</span>
                </div>
              ))}
            </div>
            ...
            <button 
              ...
              className="absolute top-3 right-3 sm:top-6 sm:right-6 text-white/20 hover:text-white transition-colors font-mono text-[10px] sm:text-xs uppercase tracking-widest"
            >
              {hudText.close}
            </button>
          </div>
...
const GlassDeviceVisual = ({ category, iconName, color = "cyan", hidden, mobile, tablet }) => {
  ...
  return (
    <div 
      className={`${sizeClasses} rounded-lg border border-white/10 bg-zinc-900/30 backdrop-blur-md flex flex-col items-center justify-center p-6 relative overflow-hidden ${hidden ? 'opacity-0 scale-95' : 'opacity-100 scale-100'} ${activeStyle} transition-all duration-300 ease-out cursor-pointer`} 
      style={{ willChange: 'transform' }}
    >
      <div className="absolute top-0 left-0 w-full h-1/2 bg-gradient-to-b from-white/10 to-transparent pointer-events-none" />
      <div className="relative z-10 mb-4">
        <div className={`absolute inset-0 blur-xl opacity-20 bg-current`} />
        {Icon && <Icon className={`${mobile || tablet ? 'w-12 h-12' : 'w-10 h-10'} relative z-10 transition-transform duration-300 group-hover:scale-110`} />}
      </div>
      <span className={`${mobile || tablet ? 'text-xs' : 'text-[10px]'} font-mono tracking-widest text-zinc-300 uppercase text-center relative z-10`}>{category}</span>
